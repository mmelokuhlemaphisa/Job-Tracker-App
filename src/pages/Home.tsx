...
export default function Home({ currentUser }: HomeProps) {
    ...
    const fetchJobs = async () => {
      try {
        const res = await fetch(
          `https://job-tracker-api-jze2.onrender.com/jobs?userId=${currentUser.id}`
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editJobId !== null) {
        // Update existing job
        await fetch(`https://job-tracker-api-jze2.onrender.com/jobs/${editJobId}`, {
          method: "PUT",
  ...
  const handleDelete = async (id: string) => {
    ...
    if (window.confirm("Are you sure you want to delete this job?")) {
      await fetch(`https://job-tracker-api-jze2.onrender.com/jobs/${id}`, {
        method: "DELETE",
      });
